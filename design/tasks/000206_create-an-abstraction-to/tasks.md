...
- [x] Implement a `StubProvider` in `internal/provider/stub.go` that satisfies the interface with in-memory/no-op behavior (useful for tests and local dev)
- [x] Implement a `StripeProvider` in `internal/provider/stripe.go` as the first real provider example (can be a skeleton with TODOs for actual Stripe SDK calls)
- [x] Create a provider registry/factory in `internal/provider/registry.go` that maps a provider name (from config) to its constructor
- [x] Add `PaymentProvider` config field to `config/config.go` (e.g., `Provider string` defaulting to `"stub"`)
